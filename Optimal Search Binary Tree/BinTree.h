...
{
	int key;//поле данных
	Node* left, * right, * parent;//указатель на левого и правого потомков и родителя

public:
	Node(int k = 0, Node* L = 0, Node* R = 0, Node* P = 0)//конструктор по умолчанию
	{
		key    = k; 
		left   = L; 
		right  = R; 
...
public:
	BinTree() { root  = new Node();; }//конструктор по умолчанию
	...
	virtual void Add(int k, Node* p);//добавить в дерево с корнем p узел с ключем k
...
//void TreeTraversal_LCR (Node *p, int *a, int *n);	
	...
	virtual Node* FindKey(int k, Node* p);//поиск узла с ключем k в дереве с корнем p
	virtual Node* FindMax(Node* p);//поиск узла с максимальным ключем в дерев с корнем p
	...
	/// 
	/// </summary>
	/// <param name="p"> корень дерева</param>
	/// <param name="k"> отступ от края консоли</param>
